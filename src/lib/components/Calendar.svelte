      ...
      {#if $viewport.isMobile}
        {#each getWeekDays(currentWeekStart) as day}
          ...
          <CalendarDayCell
            ...
            {isPast}
            {isOperator}
            ariaLabel={getDayAriaLabel(day, dayData, isPast, isMyDay)}
            onDayClick={handleDayClick}
            onContactClick={handleOperatorContactClick}
          />
        {/each}
      {:else}
...
<style>
  ...
  /* Sizing for the empty leading/trailing placeholder cells in the desktop
     month grid - CalendarDayCell.svelte owns the equivalent rule for actual
     day cells, since Svelte's scoped styles don't cross component
     boundaries. */
  .day-cell.empty {
    height: 100%;
    min-height: 120px;
